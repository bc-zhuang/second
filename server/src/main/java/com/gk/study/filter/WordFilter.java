package com.gk.study.filter;

import com.gk.study.entity.FlagIndexDTO;
...
public class WordFilter {
    ...
    public String replace(final String text, final int skip, final char symbol) {
        ...
        for (int i = 0; i < charset.length; i++) {
            FlagIndexDTO fi = getFlagIndex(charset, i, skip);
            if (fi.isFlag()) {
                if (!fi.isWhiteWord()) {
     ...
     * @param skip    文本距离
     */
    private FlagIndexDTO getFlagIndex(final char[] charset, final int begin, final int skip) {
        FlagIndexDTO fi = new FlagIndexDTO();
